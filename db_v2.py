...
# Use a separate DB file so v1 and v2 can coexist during the transition.
DB_PATH_V2 = DB_PATH.replace(".db", "_v2.db")
...
CREATE TABLE IF NOT EXISTS Task (
    id            INTEGER PRIMARY KEY AUTOINCREMENT,
    title         TEXT    NOT NULL,
    task_type     TEXT    NOT NULL DEFAULT 'task',  -- 'task' | 'list_item'
...
def init_db() -> sqlite3.Connection:
    ...
    os.makedirs(os.path.dirname(DB_PATH_V2), exist_ok=True)
    con = sqlite3.connect(DB_PATH_V2)
...
def get_annotations_for_log(con: sqlite3.Connection, log_id: int) -> list[tuple]:
    """Return all annotations for a log, ordered by position then insertion order."""
    return con.execute(
        """SELECT type, text_span, start_char, end_char, value, confidence, status
...
def get_logs(con: sqlite3.Connection) -> list[tuple]:
    """Return all logs ordered by creation time."""
    return con.execute(
        "SELECT id, raw_text, created_at, source_type, tags FROM Log ORDER BY created_at",
    ).fetchall()


def get_tasks(con: sqlite3.Connection, status: str | None = None) -> list[tuple]:
    """Return tasks ordered by task_type, status, insertion order. Filter by status if given."""
    if status:
        return con.execute(
            "SELECT id, title, task_type, status, due_date, source_log_id FROM Task "
            "WHERE status = ? ORDER BY task_type, id",
            (status,),
        ).fetchall()
    return con.execute(
        "SELECT id, title, task_type, status, due_date, source_log_id FROM Task "
        "ORDER BY task_type, status, id"
    ).fetchall()
...
def get_entity_relationships(con: sqlite3.Connection, entity_id: int) -> list[tuple]:
    """Return all relationships where entity_id is entity_a, with resolved names."""
    return con.execute("""
